...
def call_gemini_api(prompt, system_instruction=None, media=None):
    ...
    if media:
        # v1 gemini-pro は画像/音声非対応だが、エラー回避のためテキストパートのみ送るか、
        # あるいはここで例外を出す。一旦、音声データは無視してテキストのみでトライさせる。
        # (音声入力はできなくなるが、機能全体が死ぬよりマシ)
        pass 
    ...
    # v1 では responseMimeType はサポートされていないため送らない
