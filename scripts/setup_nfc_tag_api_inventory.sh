...
# NFCタグ情報取得API - 在庫管理用API Gateway設定スクリプト
# このスクリプトは、2z0ui5xfxb API Gatewayに/staff/nfc/tagエンドポイントを設定します

set -e

REGION="ap-northeast-1"
REST_API_ID="2z0ui5xfxb"  # 在庫管理用API Gateway
LAMBDA_FUNCTION_NAME="misesapo-s3-upload"
ACCOUNT_ID="475462779604"

echo "=========================================="
echo "NFCタグ情報取得API - 在庫管理用API Gateway設定"
echo "=========================================="
echo ""
...
for METHOD in GET OPTIONS; do
  ...
  aws lambda add-permission \
    --function-name ${LAMBDA_FUNCTION_NAME} \
    --statement-id "apigateway-${METHOD_LOWER}-nfc-tag-inventory-$(date +%s)-${RANDOM}" \
