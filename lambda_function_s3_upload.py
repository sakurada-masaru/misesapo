...
def _yotei_normalize_item_status(item):
    ...
    normalized['status'] = _yotei_normalize_status(item.get('status'))
    return normalized
...
def yotei_patch_dispatch(dispatch_id, event, headers):
    try:
        body = _yotei_parse_body(event)
        existing = YOTEI_DISPATCH_TABLE.get_item(Key={'id': dispatch_id}).get('Item')
        if not existing:
            return {
                'statusCode': 404,
                'headers': headers,
                'body': json.dumps({'error': 'dispatch not found'}, ensure_ascii=False)
            }
        ...
        updated = {
            ...
            **body,
            'id': dispatch_id,
            'updated_at': body.get('updated_at') or datetime.utcnow().isoformat() + 'Z'
        }
        YOTEI_DISPATCH_TABLE.put_item(Item=updated)
        return {
            'statusCode': 200,
            'headers': headers,
            'body': json.dumps({'status': 'success', 'id': dispatch_id}, ensure_ascii=False)
        ...
        return {
            'statusCode': 500,
            'headers': headers,
            'body': json.dumps({'error': 'Failed to patch yotei dispatch', 'message': str(e)}, ensure_ascii=False)
        }
