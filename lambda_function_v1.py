...
def call_gemini_api(prompt, system_instruction=None, media=None):
    ...
    # モデルとAPIバージョンを使い分ける
    # テキスト: gemini-pro (v1)
    # マルチモーダル: gemini-pro-vision (v1) - 画像メインだが一応指定
    # 注: ProVision は動画/画像用で音声単体は非対応の可能性が高いが、Flashが全滅のためこれしか選択肢がない
    ...
    if media:
        ...
        api_version = "v1"
        use_json_mode = False # v1 provision does not support json mode param
